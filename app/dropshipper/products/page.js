...
export default function DropshipperProductsPage() {
    ...
    return (
        <div className="p-4 md:p-10 space-y-10 animate-in fade-in duration-700 bg-gray-50/50 min-h-screen font-sans">
            {/* Header Section - Exactly like Admin's but with a different title */}
                                    ...
                                    if (e.target.value === "ADD_NEW") {
                                        Swal.fire({
                                            ...
                                            if (result.isConfirmed && result.value) {
                                                ...
                                                if (newCatInput) {
                                                    const normalized = newCatInput.charAt(0).toUpperCase() + newCatInput.slice(1).toLowerCase();
                                                    setCategories(prev => [...new Set([...prev, normalized])]);
                    ...
                    <table className="w-full text-left min-w-[800px]">
                        ...
                        <tbody className="divide-y divide-gray-50">
                            {loading && products.length === 0 ? (
                                <tr><td colSpan="4" className="px-8 py-20 text-center animate-pulse text-gray-400 font-bold italic">Loading inventory...</td></tr>
                            ) : products.filter(p => {
