...
export default function DropshipperLayout({ children }) {
    const [isSidebarOpen, setIsSidebarOpen] = useState(true);
    const router = useRouter();
    const pathname = usePathname();

    if (pathname === "/dropshipper/login") {
        return <>{children}</>;
    }
    ...
    const handleLogout = async () => {
        ...
        if (result.isConfirmed) {
            try {
                // Assuming there is a logout API for dropshipper or reuse admin one if it clears session
                await fetch("/api/dropshipper/logout", { method: "POST" });
                Swal.fire({
                    title: 'Logged out!',
    ...
    const navItems = [
        // { label: "Dashboard", icon: LayoutDashboard, path: "/dropshipper/dashboard" }, // Can add dashboard later if needed
        { label: "Products", icon: Package, path: "/dropshipper/products" },
    ];
        ...
        <div className="flex h-screen bg-gray-50 overflow-hidden">
            ...
            >
                ...
                <div className="p-6 flex items-center justify-between lg:justify-start gap-3 border-b lg:border-none border-gray-50">
                    <div className="flex items-center gap-3">
                        ...
                        <div className={`${!isSidebarOpen && "lg:hidden"}`}>
                            ...
                            <span className="text-[10px] text-gray-400 font-bold tracking-widest uppercase">Drop Shopper</span>
                        </div>
                    </div>
                ...
                <div className="p-4 border-t border-gray-50 bg-white">
                    ...
                    >
                        <div className="w-8 h-8 rounded-full bg-gray-900 flex items-center justify-center text-white shrink-0 group-hover:scale-110 transition-transform">
            ...
            <main className="flex-1 flex flex-col min-w-0 overflow-hidden relative h-screen">
                ...
                <div className="flex-1 overflow-y-auto w-full">
                    <div className="p-0 md:p-0 max-w-[1400px] mx-auto w-full">
                        {children}
                    </div>
                </div>
            </main>
